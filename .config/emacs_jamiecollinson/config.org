#+TITLE: Emacs Config
#+AUTHOR: Demetris Mina
#+TOC: true
#+DATE:

This is my emacs configuration. After many years with vim then a year with spacemacs & prelude, I came to the realisation that I needed to construct my own to really get it.
...
I put my scripts under =~/.config/emacs/lisp/= and symlink it with =ln -s ~/dotfiles/lisp ~/.emacs.d/lisp=.
